...
export class MissionsInterfaceComponent implements OnInit {
  ...
  constructor(private scoreService: ScoreMissionsService, private missionsService: MissionsService) {
  ...
  ngOnInit() {
    ...
    s.snapshotChanges().subscribe(data => { // Using snapshotChanges() method to retrieve list of data along with metadata($key)
      ...
      this.missionsAccomplished = this.Team[0].missionsAccomplished.split(',');
  ...
  calculeScoreTotal() {
      this.missions.forEach(mission => {
        if (this.missionsAccomplished[this.missionsAccomplished.length - 1] === mission.name) {
          this.scoreTotal = this.scoreTotal + mission.score;
        }
    });
  }
